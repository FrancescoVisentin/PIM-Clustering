...
static uint64_t max_distance = 0;

static uint64_t get_furthest_point(T* buffer, uint32_t n_points, uint32_t dim, T* candidate_center, uint64_t candidate_center_dist) {
...
int main() {
    ...
    if (tasklet_id == 0) {
        mem_reset();
        centers_set = (T*) mem_alloc(n_centers*dim*sizeof(T));

        //Il primo punto è scelto come primo centro.
        mram_read(DPU_MRAM_HEAP_POINTER, centers_set, roundup(dim*sizeof(T), 8));
        n_centers_found++;
    }
    ...
    uint32_t buffer_size = get_block_size(dim*sizeof(T));
    T* buffer = (T*) mem_alloc(buffer_size);
    
    //Indirizzo del primo blocco per ogni tasklet in MRAM.
    uint32_t points_per_block = buffer_size/(dim*sizeof(T));
    uint32_t base_block_addr = (uint32_t)(DPU_MRAM_HEAP_POINTER + buffer_size*tasklet_id);
    uint32_t last_point_addr = (uint32_t)(DPU_MRAM_HEAP_POINTER + n_points*dim*sizeof(T));

    //Buffer per contenere il candidato centro di ogni tasklet.
    T* candidate_center = (T*) mem_alloc(dim*sizeof(T));

    while (n_centers_found < n_centers) {
        uint64_t candidate_center_distance = 0;
        ...
        for(uint32_t point_index = base_block_addr; point_index < last_point_addr; point_index += buffer_size*NR_TASKLETS) {
            ...
            if (point_index + buffer_size >= last_point_addr) {
                ...
                points_block_i = (last_point_addr - point_index)/(dim*sizeof(T));
        ...
        if (tasklet_id == 0) {
            ...
            for (unsigned int i = 0; i < dim; i++) {
                centers_set[n_centers_found*dim + i] = candidate_center[i];
            }
    ...
    if (tasklet_id == 0) {
        mram_write(centers_set, DPU_MRAM_HEAP_POINTER + dpu_mem_size, roundup(n_centers*dim*sizeof(T), 8)); //TODO: mram_write può caricare 2048 bytes al massimo.        
